...
The adjusted $R^2$ of the base model is `r summary(model_additive_test)$adj.r.squared` and the $RMSE_{test}$ is `r calc_test_rmse(model_additive_test)`.
...
```
While the adjusted $R^2$ does not change much, all VIFs are below 5. Therefore, we prefer the small additive logged model with `bathrooms_comp` and `sqft_above` predictors removed compared to the previous full additive logged model.
...
Using the chosen model from BIC backwards step model selection, we look to see in there are any influential data points. Data points are considered influential if there Cook's distance is greater than $\frac{4}{n}$ where n is the size of the sample. We decide to remove these points and fit the edited data to the BIC chosen model. We see that the adjusted $R^2$ increases, the $RMSE_{test}$ decreases and the Q-Q plot improves, such that normality of the data is no longer suspect. We also see that the outlier point was removed from the Fitted vs Residuals plot allowing us to better see the linearity of the data (Fitted vs Residuals plot centered around 0).
...
While Adjusted $R^2$ `r summary(model_int_bic)$adj.r.squared` increases not considerably, the $RMSE_{test}$ `r calc_test_rmse(model_int_bic, log=TRUE)` decreases. The QQ-plot exhibits a violation of normality. We remove influential data in an attempt to improve normality and test it.
...
anova(model_bic_add_cook,model_bic_int_cook)
```

The Higher $RMSE_{test}$ of `r calc_test_rmse(model_bic_int_cook, log=TRUE)` suggests **overfitting** and Q-Q plot still exhibits a violation of normality. We discard the interactive model and kept the chosen additive model.
