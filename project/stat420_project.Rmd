...
model_additive = lm(Enrollment.rate ~ ., data = Enrollment)
...
```
```{r}
model_log = lm(Enrollment.rate ~ log(SAT.verb.75)+log(SAT.math.75)+log(ACT.engl.75)+log(ACT.math.75)+Cost.ratio+Gender.ratio+Regions+Location.type, data = Enrollment)
summary(model_log)
vif(model_log)
diagnostics(model_additive)
...
```{r}
model_poly = lm(Enrollment.rate ~ poly(SAT.verb.75, degree = 2)+poly(SAT.math.75,2)+poly(ACT.engl.75,2)+poly(ACT.math.75,2)+Cost.ratio+Gender.ratio+Regions+Location.type, data = Enrollment)
...
lambda = enroll_boxbox$'x'[which.max(enroll_boxbox$y)]

# modified additive LM with all remaining variables
model_boxcox_log = lm((((Enrollment.rate ^ lambda) - 1 ) / lambda) ~ log(SAT.verb.75)+log(SAT.math.75)+log(ACT.engl.75)+log(ACT.math.75)+Cost.ratio+Gender.ratio+Regions+Location.type, data = Enrollment)
...
model = lm(log(Enrollment.rate) ~ (.-School.ID - School.name), data = Enrollment_data)
