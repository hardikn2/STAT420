...
#house_tst = house_tst[house_tst$grade != 13,]
...
Q-Q plot looks good normality is not an suspect anymore.
...
#interactive model
model_interactive = lm(log(price) ~ (sqft_living + sqft_lot + waterfront + 
    view  + lat + long + sqft_living15 + age)^2 + grade + condition +  floors , data = house_trn)
...
```{r}
#BIC search
k = log(length(resid(model_interactive)))
model_int_bic = step(model_interactive, k= k)

summary(model_int_bic)$adj.r.squared
calc_test_rmse(model_interactive)

model_int = lm(log(price) ~ sqft_living + sqft_lot + waterfront + view + lat + 
    long + sqft_living15 + age + grade + condition + floors + 
    sqft_living:sqft_lot + sqft_living:view + sqft_living:long + 
    sqft_living:sqft_living15 + sqft_living:age + sqft_lot:lat + 
    sqft_lot:age + waterfront:long + lat:long + lat:age + long:sqft_living15 + 
    long:age + sqft_living15:age, data = house_trn)
...
model_bic_int_cook = lm(log(price) ~ sqft_living + sqft_lot + waterfront + 
    ...
    floors + sqft_living:sqft_lot + sqft_living:view + sqft_living:lat + 
    sqft_living:long + sqft_living:sqft_living15 + sqft_living:age + 
    sqft_lot:lat + sqft_lot:age + waterfront:long + lat:long + 
    lat:sqft_living15 + long:sqft_living15 + long:age + sqft_living15:age, data = house_trn, subset = cd <= 4/length(resid(model_int_bic)))
...
summary(model_int_bic)
car::vif(model_int_bic)
diagnostics(model_int_bic)
# higher "test"" RMSE suggest overfitting so we are discarding the interative model 
```
