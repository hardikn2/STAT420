...
summary(model_additive_basic)
car::vif(model_additive_basic)
calc_loocv_rmse(model_additive_basic)
calc_test_rmse(model_additive_basic)
diagnostics(model_additive_basic)
```


```{r}
model_additive_test = lm(price ~ . , data = house_tst)
summary(model_additive_test)
...
```
```{r}
model_additive_log = lm(log(price) ~ . , data = house_tst)
summary(model_additive_log)
car::vif(model_additive_log)
calc_loocv_rmse(model_additive_log)
...
model_bic = step(model_additive_log,trace=0, k= log(length(resid(model_additive_log))))
...
model_bic_add = lm(log(price) ~ sqft_living + sqft_lot + floors + waterfront + 
    ...
    bathrooms_comp + age, data = house_tst)
summary(model_bic_add)
#car::vif(model_additive)
#calc_test_rmse(model_additive,TRUE)

#model_additive = lm(log(price) ~ bedrooms + bathrooms + log(sqft_living) + sqft_lot + floors + waterfront + view + condition + grade + sqft_above + yr_built + yr_renovated + zipcode + lat + long + log(sqft_living15) + log(sqft_lot15), data= house_data_raw)


```
```{r}
cd = cooks.distance(model_bic_add)
model_bic_add_cook = lm(log(price) ~ sqft_living + floors + waterfront + view + 
    condition + lat + long + sqft_living15  + 
    age, data = house_trn, subset = cd <= 4/length(resid(model_bic_add_cook)))
summary(model_bic_add_cook)
exp(calc_loocv_rmse(model_bic_add_cook))
...
model_interactive = lm(price ~ lat + long +(sqft_living + floors + waterfront + view  + sqft_above +  sqft_living15 + sqft_lot15 + age)^2 + bedrooms_comp + bathrooms_comp + log(condition),data = house_trn)
