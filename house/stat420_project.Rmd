...
house_data_base$grade = as.factor(house_data_raw$grade)
...
house_idx = sample(nrow(house_data), 10000)
...
```{r}
model_additive = lm(price ~ . -grade,data = house_trn)
...
pca1 = prcomp(house_data_numeric, scale. = TRUE)
...
model_interactive = lm(price ~ (bedrooms + sqft_living + sqft_lot + floors + 
    waterfront + view + condition + sqft_above + yr_built + yr_renovated + 
    lat + long + sqft_living15 + sqft_lot15)^2 ,data = house_trn)
